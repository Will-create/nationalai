...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_state, writer, num_examples=2):
    ...
    # the size of the console window (just use a default value)
    console_width = 80
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            model_output_text = tokenizer_tgt.decode(model_out.detach().cpu().numpy())
...
def train_model(config):
    # Define the device
    device = torch.device('cuda' if torch.cuda.is_available() else f'cpu')
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        for batch in batch_iterator:
            ...
            global_step += 1 # Keep track of the loss

        run_validation(model, val_dataloader, tokenizer_src, tokenizer_tgt, config['seq_len'], device, lambda msg: batch_iterator.write(msg), global_step, writer)
        # Save the model a the end of every epoch
